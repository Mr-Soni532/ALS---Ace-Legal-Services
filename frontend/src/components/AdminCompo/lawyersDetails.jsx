          ...
          return (
            <div className="lawyerCard" key={el.id}>
              ...
              <div className="lawyerDetails">
                ...
                <div className="skilsDiv">{skills.map((el)=>{if(el!==""){return(<p>{el}</p>)}})}</div>
                <div className="rankDiv">
                  <img src={achievment(el.id)} alt="achievment" />
                  <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRKHs_iSXfIho850bfdEhGeYitZRpILjDpa8A&usqp=CAU" alt="" />
                </div>
              </div>
              <div className="editDltBtn">
                <button>Edit</button>
                <button onClick={()=>{deletEele(el)}}>Remove</button>
              </div>
            </div>
          );
        })}
